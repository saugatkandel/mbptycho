...
class Sample:
    ...
    def _setStephanStrain(self):
        ...
        self.Ux_full = (self.params.strain_scaling_factor * (np.cos(self.XX_full) - 1) *
                        self.obj_mask_full * np.sign(self.XX_full))
        self.Uy_full = (self.params.strain_scaling_factor * (np.cos(self.YY_full * 0.5) - 1) *
                        self.obj_mask_full * np.sign(self.YY_full))
        self.Uz_full = np.zeros_like(self.ZZ_full) * self.obj_mask_full * np.sign(self.ZZ_full)
    ...
    def setSampleRhos(self,
        ...
        displacements_full = np.array([self.Ux_full, self.Uy_full, self.Uz_full]).reshape(3, -1)
    ...
    def truncateNumericalWindow(self, pad=2, xy_scan=True) -> np.ndarray:
        ...
        """
        raise NotImplementedError("Scipy interpolation routine is wonky for coordinates near numerical window edges.")
        if not xy_scan:
        ...
        slice_trunc = np.s_[self.nw_ymin: self.nw_ymax,
                self.nw_xmin: self.nw_xmax,
                self.nw_zmin: self.nw_zmax]
        
        slice_delta = np.s_[self.nw_ymin_delta: self.nw_ymax_delta,
...
class SimulationPerBraggPeak:
    def __init__(self, rho: np.ndarray,
        ...
        self.rho = rho
        #self.rho_full = rho_full
        #self.probe_untrunc = probe
        #self.probe_full= probe_full
        self.probe = probe
        ...
        self.YY_full, self.XX_full, self.ZZ_full = np.meshgrid(self.y_full, self.x_full, self.z_full, indexing='ij')
        self.nw_coords_stacked = np.stack((self.YY_full.flatten(), self.XX_full.flatten(), self.ZZ_full.flatten()), axis=0)
        ...
        self.nw_rotation_mask_delta = self.getNonzeroMaskAfterRotation(self.nw_rotated_coords_stacked,
                                                                 self.sample_params.grain_height_delta,
                                                                 self.sample_params.grain_width_delta,
                                                                 self.sample_params.film_thickness,
                                                                 self.sample_params.sample_pix_size)
        self.nw_rotated_masked_delta_coords = self.nw_rotated_coords_stacked.T[self.nw_rotation_mask_delta].T
        ...
        [
            ...
            #self.probe_slices,
            self.projection_all
        ] = self.getDiffractionPatternsProbeRoll(poisson_noise=poisson_noise, poisson_noise_level=poisson_noise_level)
    ...
    def getDiffractionPatternsProbeRoll(self, poisson_noise: bool = False, poisson_noise_level: float = 0):
        ...
        diffraction_patterns = np.zeros((self.ptycho_scan_positions.shape[0],
                                         self.npix_det, self.npix_det))
        ...
        proj_slice = np.s_[npix_diffy: rs[0] - npix_diffy, npix_diffx: rs[1] - npix_diffx]
        ...
        for indx, (pcy, pcx) in enumerate(self.ptycho_scan_positions):
            ...
            probe_this = np.pad(self.probe, [[pady0, pady1], [padx0, padx1], [0, 0]], mode='constant')[slice_this]
            ...
            rgi = RegularGridInterpolator((self.y_full, self.x_full, self.z_full),
                                          field,
                                          method='linear',
                                          bounds_error=False,
                                          fill_value=0)
            ...
            # volume of interest.
            projection = projection[proj_slice]#[npix_diffy: rs[0] - npix_diffy, npix_diffx: rs[1] - npix_diffx]
...
class Simulation:
    def __init__(self, sim_params: dict = None,
        ...
        if not hasattr(self, "sample"):
            ...
            self.sample = Sample(wavelength=self.params.wavelength, sample_pix_size=self.params.sample_pix_size,
