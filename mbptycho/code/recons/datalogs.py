...
@dt.dataclass
class CustomFunctionMetric(SimpleMetric):
    func: Callable
...
class DataLogs:
    ...
    def addCustomFunctionMetric(self, title: str,
                              func: Callable,
                              log_epoch_frequency,
                              **kwargs):

        self._datalog_items.append(CustomFunctionMetric(title=title,
                                                      func=func,
    ...
    def getCustomFunctionMetrics(self, epoch):
        funcs = {}
        for item in self._datalog_items:
            if isinstance(item, CustomFunctionMetric):
                if epoch % item.log_epoch_frequency == 0:
                    funcs[item.title] = item.func
        return funcs
