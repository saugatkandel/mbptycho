...
#from mbptycho.code.recons.reconstruction import ReconstructionT


class ProjectedDisplacementReconstructionT:
    def __init__(self, simulation: Simulation,
        ...
        with tf.device('/gpu:0'):
            ...
            magnitudes_log_init = np.log(magnitudes_init + 1e-8)
    ...
    def getMagnitudes2D(self, magnitudes_log_v):
        magnitudes = tf.exp(magnitudes_log_v)
        if self.shared_magnitudes:
            magnitudes_2d_profile = tf.reshape(magnitudes[:self.npix_xy], (self.npix_y, self.npix_x))
            magnitudes_scaling_vars = magnitudes[self.npix_xy:]
            ...
            magnitudes_2d_all = tf.reshape(magnitudes, (-1, self.npix_y, self.npix_x))
    ...
    # @tf.function
    def _batchPredictFromDisplacementsMagnitudes(self, ux_uy_2d_v, magnitudes_log_v):
    ...
    # @tf.function
    def _batchPredictFromPhasesMagnitudes(self, phases_v, magnitudes_log_v):
    ...
    def displacementPredictFn(self, ux_uy_2d_v):
        return self._batchPredictFromDisplacementsMagnitudes(ux_uy_2d_v, self.magnitudes_log_v)

    def magnitudesPredictFn(self, magnitudes_log_v):
        if self.reconstruction_type == "displacement":
            return self._batchPredictFromDisplacementsMagnitudes(self.ux_uy_2d_v, magnitudes_log_v)
        else:
            return self._batchPredictFromPhasesMagnitudes(self.phases_v, magnitudes_log_v)
    ...
    def loss_fn(self, predicted_data_t):
        ...
        return 0.5 * tf.reduce_sum((batch_diff_data_t - predicted_data_t) ** 2) / self.batch_size
    ...
    def objective_fn(self):
        return self.loss_fn(self._batchPredictFromPhasesMagnitudes(self.phases_v, self.magnitudes_log_v))
    ...
    @tf.function
    def optimizersMinimize(self, iters_before_check: tf.Tensor):
